...
def read_profiles(db: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    return get_profile(db=db)

@router.put("/profile", response_model=Profile)
def create_profile(profile_in: ProfileCreate, db: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    return create_profile(db=db, profile_in=profile_in)

# @router.put("/profile/{profile_id}", response_model=Profile)
# def update_profile(profile_id: int, profile_in: ProfileUpdate, db: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
